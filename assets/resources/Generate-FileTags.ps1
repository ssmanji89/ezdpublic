...
foreach ($file in $ps1Files) {
    ...
    $fileContent=    gc (($path)+"\Get-FilesOver1000MB.ps1") -Raw -Encoding UTF8; 
    $fileContent = (($fileContent).replace('`n',' ')).Trim() #| ForEach-Object {$fileString = $fileString + $_.ToString()}
    $dataBack = (gpt -Verbose -prompt "Generate a set of Tags using ChatGPT for a provided. `n "+$fileContent.ToString+"").choices.text
